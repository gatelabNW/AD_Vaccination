...
# Summary: Meninges Cell2Location Step 2
#
#-------------------------------------------------------------------------------
...
dat = sc.read_h5ad("mng/all_sample_sc_after_training_1223.h5ad")
mng_ref_downsampled = sc.read_h5ad("mng_1223.h5ad")
...
for sample in dat.obs["sample_id"].unique():
  ...
  inf_aver = pd.read_csv("/C2Lresults/inf_aver_mng_1223.csv")

  # index vals saved in column 1 but not index so make sure inf_aver.index = mng_ref_downsamled.var_names before running the model
 ...
 # Save model
  ...
  plt.close()
  #############################################################################################################
  # plt.savefig(f"{results_folder}/{sample}_model_qc_0714.png")
