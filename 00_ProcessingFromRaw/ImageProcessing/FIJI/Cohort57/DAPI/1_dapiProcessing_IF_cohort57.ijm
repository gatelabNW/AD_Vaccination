...
function dapiThresholding(currentDir) {
    ...
    for (i = 0; i < fileList.length; i++) {
		...
		run("Convert to Mask");
		saveAs("Tiff", newDirPath + "/" + sampleName + "-dapi-binary.tif");
		
		// Analyze Particles
		selectImage(sampleName + "-dapi-binary.tif");
		run("Analyze Particles...", "size=0-10000 pixel show=Masks clear overlay");
			// adjust upper pixel bound depending on image resolution
		run("Invert");
		run("Watershed");
		saveAs("Tiff", newDirPath + "/" + sampleName + "-dapi-particles-watershed.tif");
