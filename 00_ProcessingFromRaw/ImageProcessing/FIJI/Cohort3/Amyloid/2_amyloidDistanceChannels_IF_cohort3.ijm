...
function amyloidDistanceChannels(amyloidBinary, outputDir) {
	...
	for (i = 0; i < amyloidFileList.length; i++) {
		...
		imageCalculator("Add create", current, d1st);
		...
		saveAs("tiff", outputDir + type + "_" + sampleName + "_cleaned_map");
		print("Done with " + type + "_" + sampleName);
		close("*");
	}
}

function nextDilation(counter, latestImage, newestImage, iteration){
	run("Duplicate...", "title=[dilate_" + iteration*60 + "x]");
	duplicate = getTitle();
...
// run it!
amyloidDirectory = getDirectory("Navigate to the binary amyloid images ");
