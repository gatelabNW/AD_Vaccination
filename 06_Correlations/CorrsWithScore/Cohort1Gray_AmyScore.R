...
# Summary: Cohort 1 gene correlations with combined microglia-amyloid score
#
#-------------------------------------------------------------------------------
...
# Add C2L microglia data
s@meta.data$microglia <- 0

# Create list for microglia data
gliadat <- list()
...
# calculate score
s@meta.data$newamyscore <- sqrt((s@meta.data$distance_inv_scaled)^2 + (s@meta.data$microglia_scaled)^2)
...
for(condition_group in names(mats)){
  ...
  covs[[condition_group]] <- metas[[condition_group]][,c(
    # "age_centered", 
    #                                                      "gDNA_percent_centered",
    "cdr_centered" 
    # covcols
  )]
  ...
  for(gene in rownames(mat)){
    ...
    ctest <- pcor.test(x = metas[[condition_group]]$newamyscore, 
  ...
  # Save CSV of cors
  write.csv(resmat[[paste0(condition_group)]], file = paste0(condition_group, "_Score_Corr.csv"))
...
# Print upset plot
pdf(file = "AmyGliaScoreSigGenesUpset.pdf", width = 12, height = 8)
print(p)
dev.off()
...
# Only if there are very few sig genes and the labels won't become overwhelming
...
# sharedUnique3 replaces genes also sig in nAD with "Sig in nAD"
...
p <- ggplot(data = plotgenes, 
  ...
  ylab("-log10(padj)") + 
  xlab("Spearman's Rho") + 
  ggtitle("All Genes | SCT Data | Glia-Plaque Score | Gray Matter | 125") +
...
# save plot
pdf(file = "AmyGliaScoreSigGenes.pdf", width = 24, height = 12)
print(p)
dev.off()

# save data
write.csv(plotgenes, file = "AmyGliaScoreSigGenes.csv")