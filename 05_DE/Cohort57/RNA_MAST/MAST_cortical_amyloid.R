...
# Summary: Cortical amyloid-rich differential expression with MAST (Fig 4E, Ext Fig 4E)
#
#-------------------------------------------------------------------------------
# Initialization
...
output_folder <- "/path/to/cortical/amyloid/MAST/results/"
...
# Subset for spots with no vascular amyloid signal 
s <- subset(s, vessel_fluo == 0)

# Define amyloid enrichment 
s$amyloid_rich <- "not_rich"
s$amyloid_rich[s$plaque_fluo > 153] <- "rich"
  ...
  tryCatch(
    {
      ...
      write.csv(results, paste0(cur_folder, "mast_results_sct_cdr.csv"))
    },
    error = function(e) {
