...
# Summary: Differential Expression with DESeq2 for C2L enriched cell types in gray matter (Fig 2D-E)
#
#-------------------------------------------------------------------------------
...
output_folder <- "/path/to/graymatter/celltype/deseq/results/"
...
# Subset for gray matter 
s <- subset(s, manual_annotation %notin% c("white", "meninges", ""))
...
for (comparison in comparisons) {
  ...
  for (type in names(bulk_counts)) {
    ...
    # If comparing iAD-Extensive or iAD-Limited, override condition variable with condition_clearance
    if ((ident.1 %in% c("ext", "lim")) | (ident.2 %in% c("ext", "lim"))) {
